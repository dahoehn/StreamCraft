...
sc::MS_TARGETS_SPECTRA sc::MS_ANALYSIS::get_spectra_targets(const sc::MS_TARGETS &targets, const double &minIntLv1 = 0, const double &minIntLv2 = 0)
{
  ...
  MS_SPECTRA_HEADERS headers = get_spectra_headers();
...
#pragma omp for
    for (int i = 0; i < number_spectra_targets; i++)
    {
      ...
      const double &i_pre_mz = headers.precursor_mz[i_idx[0]];
      const double &i_pre_mzlow = headers.window_mzlow[i_idx[0]];
      const double &i_pre_mzhigh = headers.window_mzhigh[i_idx[0]];
      const double &i_rt = headers.rt[i_idx[0]];
      const double &i_mobility = headers.mobility[i_idx[0]];
