...
double sc::mzml::MZML_SPECTRUM::extract_scan_mobility() const
{
  ...
  if (mobility_node)
    return mobility_node.attribute("value").as_double();
...
std::string sc::mzml::MZML::get_type()
{
  ...
  if (number_spectra > 0)
  {
    ...
    bool no_pre_mz = std::all_of(pre_mz.begin(), pre_mz.end(), [](double d)
                                 { return std::isnan(d); });
    bool no_pre_mzhigh = std::all_of(pre_mzhigh.begin(), pre_mzhigh.end(), [](double d)
                                     { return std::isnan(d); });
    if (level.size() > 1)
    {
      if (no_pre_mz)
